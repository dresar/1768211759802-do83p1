...
function extendSocket(sock) {
    ...
    sock.getName = async (jid, groupJid = null) => {
        ...
        if (isLid(jid) || isLidConverted(jid)) {
            ...
            if (cached) {
                ...
                try {
                    ...
                    resolvedJid = resolveAnyLidToJid(jid, groupMeta.participants || []);
                } catch (e) {
                    resolvedJid = jid.replace('@lid', '@s.whatsapp.net');
                }
            } else {
                resolvedJid = jid.replace('@lid', '@s.whatsapp.net');
            }
        ...
        try {
            if (sock.store?.contacts?.[resolvedJid]) {
                const contact = sock.store.contacts[resolvedJid];
        ...
        try {
            const status = await sock.fetchStatus(resolvedJid);
            if (status?.status) {
            }
        } catch (e) {}
