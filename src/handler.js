...
async function handleSmartTriggers(m, sock, db) {
    ...
    try {
        ...
        const thumbPath = './assets/images/thumbnail2.jpg'
        try {
        ...
        if (m.isGroup && (text?.toLowerCase() === 'bot' || text?.toLowerCase().includes('invywed'))) {
            await sock.sendMessage(m.chat, {
                text: `🤖 *ʙᴏᴛ ᴀᴋᴛɪꜰ*\n\n` +
...
async function messageHandler(msg, sock) {
    try {
        ...
        const db = getDatabase();
        if (!db?.ready) {
            try { logger.debug('Handler', 'Skipped: database not ready') } catch (e) {}
            return;
        }
        ...
        if (!modeCheck.allowed) {
            if (modeCheck.isAfk && m.isCommand) {
                ...
                try { logger.debug('Handler', 'Skipped: restricted by mode') } catch (e) {}
            }
            return; 
        }
