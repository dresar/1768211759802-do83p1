...
async function startConnection(options = {}) {
    ...
    if (usePairingCode && !sock.authState.creds.registered) {
        ...
        try {
            ...
            const code = await sock.requestPairingCode(phoneNumber, "INVYWED");
            console.log('');
...
sock.ev.on('connection.update', async (update) => {
        ...
        setTimeout(async () => {
            ...
            try {
                if (messageQueue.length > 0) {
                    ...
                    colors.logger.info('Message', `Flushing queued batches: ${queued.length}`);
                    for (const batch of queued) {
                        await processUpsertBatch(batch.messages, batch.type);
                    }
                }
            } catch (e) {
                colors.logger.error('Message', `Flush error: ${e.message}`);
            }
    ...
    async function processUpsertBatch(messages, type) {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        if (!connectionState.isReady) {
            try {
                messageQueue.push({ messages, type });
                colors.logger.debug('Message', `Queued: isReady=${connectionState.isReady}, size=${messageQueue.length}`);
            } catch (e) {}
            return;
        }
        
        await processUpsertBatch(messages, type);
    });
