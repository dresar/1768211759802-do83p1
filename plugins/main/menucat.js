...
try {
    let thumbPath = path.join(process.cwd(), 'assets', 'images', 'thumbnail.jpg')
    if (!fs.existsSync(thumbPath)) {
        thumbPath = path.join(process.cwd(), 'assets', 'images', 'thumbnail.png')
    }
...
async function handler(m, { sock }) {
    ...
    if (!categoryArg) {
        ...
        const categoryOrder = ['owner', 'main', 'utility', 'tools', 'fun', 'game', 'download', 'search', 'sticker', 'media', 'ai', 'group', 'religi', 'info', 'jpm', 'pushkontak', 'panel', 'user', 'ephoto', 'store']
        ...
        for (const cat of sortedCats) {
            ...
            txt += `┃ ${emoji} *${toMonoUpperBold(cat)}* ┃ \`${cmds.length}\` cmds\n`
        }
        txt += `╰┈┈┈┈┈┈┈┈⬡\n\n`
    ...
    const matchedCat = categories.find(c => c.toLowerCase() === categoryArg)
    ...
    const commands = commandsByCategory[matchedCat] || []
    ...
    txt += `╰┈┈┈┈┈┈┈┈⬡\n\n`
    txt += `> Total: \`${commands.length}\` commands\n`
