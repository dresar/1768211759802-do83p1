...
\n`
    ...
    txt += `🤖 *ʙᴏᴛ ɪɴꜰᴏ* 」\n`;
    txt += `◦ ɴᴀᴍᴀ: *${botConfig.bot?.name || 'Invywed Bot'}*\n`;
    txt += `◦ ᴠᴇʀsɪ: *v${botConfig.bot?.version || '1.7.0'}*\n`;
    ...
    txt += `◦ ᴏᴡɴᴇʀ: *${botConfig.owner?.name || 'Eka'}*\n`;
    txt += `\n`;
...
function getContextInfo(botConfig, m, thumbBuffer, renderLargerThumbnail = false) {
    ...
    const ctx = {
        ...
        externalAdReply: {
            ...
            body: `ᴠ${botConfig.bot?.version || '1.7.0'} • ${(botConfig.mode || 'public').toUpperCase()}`,
            sourceUrl: '',
            mediaType: 1,
...
function getVerifiedQuoted(botConfig) {
    ...
    return {
        ...
        message: {
            extendedTextMessage: {
                text: `✨ *${botConfig.bot?.name || 'Invywed Bot'}* ✨\nꜰᴀsᴛ ʀᴇsᴘᴏɴsᴇ ʙᴏᴛ`,
                contextInfo: {
                    isForwarded: true,
                    forwardingScore: 9999
                }
...
async function handler(m, { sock, config: botConfig, db, uptime }) {
    ...
    // Text-only response: no images, no links, no extras
    // await m.reply(text);
    // return;
    
    const imagePath = path.join(process.cwd(), 'assets', 'images', 'thumbnail.jpg');
    const thumbPath = path.join(process.cwd(), 'assets', 'images', 'thumbnail2.jpg');
    const videoPath = path.join(process.cwd(), 'assets', 'video', 'bot.mp4');
    ...
    // Fitur Musik 1x (di-nonaktifkan sesuai permintaan)
    // saluran/channel (di-nonaktifkan sesuai permintaan)
    ...
    try {
        switch (menuVariant) {
            ...
            case 3:
                ...
                try {
                    const ourinPath = path.join(process.cwd(), 'assets', 'images', 'thumbnail.jpg');
            ...
            case 5:
                ...
                try {
                    ...
                    const msgOptions = {
                        footer: `© ${botConfig.bot?.name || 'Invywed Bot'} | ${sortedCats.length} Categories`,
                    ...
                    if (saluranId) {
                        msgOptions.contextInfo.forwardedNewsletterMessageInfo = {
                    ...
                    if (imageBuffer) {
                        msgOptions.image = imageBuffer;
                        msgOptions.caption = headerText;
                    } else {
                        msgOptions.text = headerText;
                    }

                    await sock.sendMessage(m.chat, msgOptions, { quoted: getVerifiedQuoted(botConfig) });
            ...
            case 6:
                ...
                const contextInfoV6 = {
                    ...
                    isForwarded: true,
                    externalAdReply: {
                        title: botConfig.bot?.name || 'Invywed Bot',
