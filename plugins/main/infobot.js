...
async function handler(m, { sock, config: botConfig, db, uptime }) {
    ...
    infoText += `│  ◦ \`Nama\`: ${botConfig.bot?.name || 'Invywed Bot'}\n`;
    infoText += `│  ◦ \`Versi\`: ${botConfig.bot?.version || '1.0.0'}\n`;
    infoText += `│  ◦ \`Developer\`: ${botConfig.bot?.developer || 'Invywed Team'}\n`;
    ...
    const contextInfo = {
        ...
        externalAdReply: {
            ...
            thumbnail: fs.existsSync('./assets/images/thumbnail.jpg') ? fs.readFileSync('./assets/images/thumbnail.jpg') : undefined,
