...
try {
    ...
    if (fs.existsSync(path.join(assetsPath, 'thumbnail.jpg'))) {
        thumbNight = fs.readFileSync(path.join(assetsPath, 'thumbnail.jpg'))
        thumbDay = fs.readFileSync(path.join(assetsPath, 'thumbnail.jpg'))
    } else if (fs.existsSync(path.join(assetsPath, 'thumbnail.png'))) {
        thumbNight = fs.readFileSync(path.join(assetsPath, 'thumbnail.png'))
        thumbDay = fs.readFileSync(path.join(assetsPath, 'thumbnail.png'))
    }
...
function getWWContextInfo(title = '🐺 WEREWOLF', body = 'Social deduction game!', thumbBuffer = thumbWW) {
    const contextInfo = {
    ...
    const commands = {
        ...
        join: async () => {
            ...
            await sock.sendMessage(m.chat, {
                ...
                contextInfo: getWWContextInfo('🐺 WEREWOLF', `${ww[m.chat].players.length} players`)
            }, { quoted: m })
        },
        ...
        try {
            ...
            if (text) {
                await sock.sendMessage(player.id, {
                    text,
                    mentions,
                    contextInfo: getWWContextInfo('🌙 NIGHT', 'Gunakan skillmu!', thumbNight)
                })
            }
        } catch (e) {
...
async function processNightActions(chatId, sock, db, prefix) {
    ...
    if (winner) {
        await sock.sendMessage(chatId, {
            ...
            contextInfo: getWWContextInfo('☀️ DAY', 'Pagi telah tiba...', thumbDay)
        })
    ...
    await sock.sendMessage(chatId, {
        text: nightReport,
        mentions: ww[chatId].players.map(p => p.id),
        contextInfo: getWWContextInfo('☀️ DAY', 'Voting time!', thumbDay)
    })
